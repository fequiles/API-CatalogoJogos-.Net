...
namespace ExemploApiCatalogoJogos.Repositories
{
    public class JogoSqlServerRepository : IJogoRepository
    {
        ...
        public async Task<List<Jogo>> Obter(string nome, int ano)
        {
            var jogos = new List<Jogo>();

            var comando = $"select * from Jogos where Nome = '{nome}' and Year = '{ano}'";
        ...
        {
            var comando = $"insert Jogos (Id, Nome, Produtora, Preco, Ano) values ('{jogo.Id}', '{jogo.Nome}', '{jogo.Produtora}', '{jogo.Preco}', {jogo.Ano.ToString().Replace(",", ".")})";
        ...
        {
            var comando = $"update Jogos set Nome = '{jogo.Nome}', Produtora = '{jogo.Produtora}', Preco = , '{jogo.Preco}', Ano = {jogo.Ano.ToString().Replace(",", ".")} where Id = '{jogo.Id}'";
